    ...
    for tweet in all_tweets:
        ...
        if len(hashtags_of_tweet) > 0:
            for hashtag in hashtags_of_tweet:
                ...
                found_entry = next((item for item in hashtags_list_of_dicts if item['hashtag'] == text), None)
                if found_entry != None:
                    found_entry['count'] = found_entry['count'] + 1
                else:
                   hashtag_dict = dict.fromkeys(['hashtag','count'])
                   hashtag_dict['hashtag'] = text
                   hashtag_dict['count'] = 1
                   hashtags_list_of_dicts.append(hashtag_dict)
    sorted_list = sort_list(hashtags_list_of_dicts)
    return sorted_list   
